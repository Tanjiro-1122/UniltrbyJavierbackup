...
export default function ChatPage() {
  ...
  /* ─── RELOAD ON RETURN (from Settings, etc.) ─── */
  useEffect(() => {
    const reloadFromStorage = () => {
      const c = localStorage.getItem("unfiltr_companion");
      const e = localStorage.getItem("unfiltr_env");
      const v = localStorage.getItem("unfiltr_vibe");
      // Always force-update companion, env, and vibe from localStorage
      if (c) {
        const parsed = JSON.parse(c);
        const savedNickname = localStorage.getItem("unfiltr_companion_nickname");
        parsed.displayName = (savedNickname && savedNickname.trim()) ? savedNickname.trim() : parsed.name;
        setCompanion(parsed);
      }
      if (e) setEnvironment(JSON.parse(e));
      if (v) setVibe(v);
    };
      ...
      >
        ...
        <div style={{ position: "relative", zIndex: 1, display: "flex", flexDirection: "column", height: "100%" }}>
              ...
              {!isPremium ? (
                remaining <= 5 ? (
                  <button onClick={() => setShowPaywall(true)}
                    style={{ marginTop: 3, fontSize: 10, color: "#f87171", background: "rgba(239,68,68,0.15)", border: "1px solid rgba(239,68,68,0.3)", padding: "2px 8px", borderRadius: 999, cursor: "pointer", animation: "none" }}>
          ...
          <div style={{ flexShrink: 0, display: "flex", flexDirection: "column", maxHeight: "52%", background: "linear-gradient(to bottom, rgba(8,3,16,0) 0%, rgba(8,3,16,0.9) 10%, rgba(8,3,16,0.97) 100%)" }}>
            ...
            {/* bottom safe area handled by BottomTabs */}
          </div>
