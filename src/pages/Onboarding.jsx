...
export default function Onboarding() {
  ...
  const handleNext = async () => {
    ...
    if (step === 3) {
      ...
      // Always save to localStorage first so navigation works even if DB fails
      ...
      // Try to persist to DB — but don't block navigation if it fails
      try {
        ...
        // DB save failed (e.g. not logged in on web) — generate local IDs and continue
        console.warn("DB save skipped:", err.message);
        const localId = "local_" + Date.now();
        localStorage.setItem("userProfileId", localId);
        localStorage.setItem("companionId",   localId + "_companion");
      } finally {
        setLoading(false);
      }
      ...
      <AnimatePresence mode="wait">
        ...
        {step === 0 && (
          <motion.div key="s0"
            ...
            style={{ flex: 1, minHeight: 0, display: "flex", flexDirection: "column", padding: "8px 16px 0", position: "relative", zIndex: 1 }}
          >
