...
// ─── Google Sign-In handler (Android only) ─────────────────────────────────────
async function handleGoogleSignIn({ googleUserId, email, displayName, isPremiumFromRC }) {
  const res = await fetch("/api/syncProfile", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({
  ...
  if (profile?.profileId) {
    localStorage.removeItem("unfiltr_fresh_start");
    localStorage.removeItem("unfiltr_signed_out");
    // Store Google user ID — web app uses unfiltr_apple_user_id as the universal auth key
    localStorage.setItem("unfiltr_google_user_id", googleUserId);
    localStorage.setItem("unfiltr_apple_user_id", googleUserId); // backward compat
    localStorage.setItem("unfiltr_user_id", googleUserId);
    if (email) {
      localStorage.setItem("unfiltr_user_email", email);
      localStorage.setItem("unfiltr_apple_email", email);
    }
    ...
    if (profile.display_name || displayName) {
      localStorage.setItem("unfiltr_display_name", profile.display_name || displayName);
    }
    if (profile.companion_id && profile.companion_id !== "pending") {
      localStorage.setItem("unfiltr_companion_id", profile.companion_id);
    }
  ...
  if (!isNewUser && profile?.companion) {
    ...
    if (comp.nickname) localStorage.setItem("unfiltr_companion_nickname", comp.nickname);
    const companionId = (profile.companion_id && profile.companion_id !== "pending")
      ? profile.companion_id : comp.avatar_id;
    if (companionId) {
      try {
        const { COMPANIONS: COMP_LIST } = await import("@/components/companionData");
        const found = COMP_LIST.find(c => c.id === companionId);
...
export default function HomeScreen() {
  ...
  const isAndroid = isNative && !!window.__isAndroid;
  ...
  const handleGoogleSignInClick = () => {
    ...
    const handleMsg = async (msg) => {
      if (resolved) return;
      if (msg.type !== "GOOGLE_SIGN_IN_SUCCESS" &&
          msg.type !== "GOOGLE_SIGN_IN_CANCELLED" &&
          msg.type !== "GOOGLE_SIGN_IN_ERROR") return;

      finish();

      if (msg.type === "GOOGLE_SIGN_IN_SUCCESS") {
        const googleUserId  = msg.googleUserId;
        const email         = msg.email || "";
        const displayName   = msg.displayName || "";
        const isPremiumRC   = msg.isPremium || false;

        debugLog(`[HomeScreen] ✅ Google Sign-In: ${googleUserId}`);
        ...
        try {
          const { profile, isNewUser } = await handleGoogleSignIn({
            googleUserId, email, displayName, isPremiumFromRC: isPremiumRC,
          });
          ...
          debugLog(`[HomeScreen] Google DB error: ${e.message}`);
          navigate("/onboarding/consent", { replace: true });
        }

      } else if (msg.type === "GOOGLE_SIGN_IN_CANCELLED") {
        debugLog("[HomeScreen] Google sign-in cancelled");
        setLoading(false);
      } else if (msg.type === "GOOGLE_SIGN_IN_ERROR") {
        debugLog(`[HomeScreen] Google sign-in error: ${msg.error}`);
    ...
    debugLog("[HomeScreen] Sending SIGN_IN_WITH_GOOGLE to native...");
    bridge.postMessage(JSON.stringify({ type: "SIGN_IN_WITH_GOOGLE" }));
  };
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        {/* Sign in button — Apple on iOS, Google on Android, Get Started on web */}
        {isNative && !isAndroid ? (
          // iOS — Apple Sign-In
          <motion.button
          ...
          <motion.button
            ...
            style={{
              ...
              border: "none", borderRadius: 20,
              color: "#1f1f1f", fontWeight: 800, fontSize: 17,
              cursor: loading ? "default" : "pointer",
              display: "flex", alignItems: "center", justifyContent: "center", gap: 10,
              marginBottom: 12, transition: "all 0.2s",
            }}>
            {loading ? (
              <>
                <div style={{
                  width: 20, height: 20, border: "2px solid #1f1f1f",
                  borderTopColor: "transparent", borderRadius: "50%",
                  animation: "spin 0.8s linear infinite"
                }} />
                <span>Signing in...</span>
